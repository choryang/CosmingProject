    ...
    return (
        <View style={{flex: 1, backgroundColor: '#b0c1e821', paddingHorizontal: 20}}>
            ...
            <View style={{flex: 1}}>
                ...
                <TouchableOpacity activeOpacity={1} style={(isVisible == 2) ? styles.ItemSelect : styles.Item} onPress={setVisible2}>
                    <Text style={(isVisible == 2) ? styles.TextSelect : styles.ItemText}>Q. 코스밍의 성분정보는 어디서 가져왔나요?</Text>
                    {(isVisible == 2) ? <Text style={styles.TextSelect}>▲</Text> : <Text style={styles.ItemText}>▼</Text>}
                </TouchableOpacity>
                {(isVisible == 2) &&
                <View>
                ...
                <TouchableOpacity activeOpacity={1} style={(isVisible == 3) ? styles.ItemSelect : styles.Item} onPress={setVisible3}>
                    <Text style={(isVisible == 3) ? styles.TextSelect : styles.ItemText}>Q. 성분이 인식되지 않아요.</Text>
                    {(isVisible == 3) ? <Text style={styles.TextSelect}>▲</Text> : <Text style={styles.ItemText}>▼</Text>}
                </TouchableOpacity>
                {(isVisible == 3) &&
                <>
                ...
                <TouchableOpacity activeOpacity={1} style={(isVisible == 4) ? styles.ItemSelect : styles.Item} onPress={setVisible4}>
                    <Text style={(isVisible == 4) ? styles.TextSelect : styles.ItemText}>Q. 코스밍의 정보가 잘못됐어요! 고쳐주세요.</Text>
                    {(isVisible == 4) ? <Text style={styles.TextSelect}>▲</Text> : <Text style={styles.ItemText}>▼</Text>}
                </TouchableOpacity>
                {(isVisible == 4) &&
                <Text style={styles.TextArticle}>코스밍의 정보가 잘못되었다면 개발자 이메일(okee0815@gmail.com)로 연락주시면 빠른시일내에 수정하도록 하겠습니다. 불편을 드려 죄송합니다.</Text>}

                <TouchableOpacity activeOpacity={1} style={(isVisible == 5) ? styles.ItemSelect : styles.Item} onPress={setVisible5}>
                    <Text style={(isVisible == 5) ? styles.TextSelect : styles.ItemText}>Q. 오류가 발생했어요. 어떻게 해야 하나요?</Text>
                    {(isVisible == 5) ? <Text style={styles.TextSelect}>▲</Text> : <Text style={styles.ItemText}>▼</Text>}
                </TouchableOpacity>
                {(isVisible == 5) &&
                <>
